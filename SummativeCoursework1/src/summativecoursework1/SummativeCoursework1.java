package summativecoursework1;

import java.util.*;

public class SummativeCoursework1 {
...
// Iterate through the diceAside array to update it with selected dice values
        for (int i = 0; i < diceAside.length; i++) {
            // Check if the current element in diceAside is 0 (indicating an empty spot)
            if (diceAside[i] == 0) {
                // Initialize a variable to store the parsed input dice value
                int diceA = -1;
                try {
                    // Attempt to parse the next input dice value from the inputDice array
                    diceA = Integer.parseInt(inputDice[idx]);
                } catch (NumberFormatException e) {
                    // Catch a NumberFormatException if parsing fails (e.g., input is not a valid integer)
                    System.out.println("Invalid...");
                }

                // Check if parsing was successful (diceA is not -1)
                if (diceA != -1) {
                    // Assign the parsed value to the current position in diceAside
                    diceAside[i] = diceA;
                    // Increment idx to move to the next input dice value in the inputDice array
                    idx++;
                }
            }
        }
